...
public class MainOfferAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private Context mcontext;
    private List<Offer> adsList;

    public MainOfferAdapter(Context context, List<Offer> adsList) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        final Offer toplist = adsList.get(position);
        ((ItemViewHolder) holder).btn_shop.setVisibility(View.VISIBLE);
        ...
        if (isProduct.equals("1")) {
            ((ItemViewHolder) holder).txt_saveup_to.setText(mcontext.getString(R.string.save_upto) + " " + toplist.getFixed() + " " + mcontext.getString(R.string.offf));
        } else {
